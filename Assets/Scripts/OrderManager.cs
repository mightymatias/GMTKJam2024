...
public class OrderManager : MonoBehaviour
{
    ...
    public void NewOrder()
    {
        ...
        UnityEngine.Vector2 spawnPoint = new UnityEngine.Vector2(PrefabX, PrefabY);
        GameObject newOrderCard = Instantiate(
            orderCardPrefab,
            transform.position,
        ...
        newOrderCard.transform.Find("Final Product").GetComponent<SpriteRenderer>().sprite =
            randomRecipe.product.GetComponent<SpriteRenderer>().sprite;
        ...
        foreach (GameObject ingredient in randomRecipe.ingredients)
        {
            ...
            cardIngredients[i].GetComponent<SpriteRenderer>().sprite = ingredient
        ...
        foreach (GameObject order in currentOrders)
        {
            Debug.Log(
                "You gave: "
                    + product
                    + " Order called for: "
                    + order.GetComponent<Order>().recipe.product
            );
            if (
                order.GetComponent<Order>().recipe.GameObject().GetComponent<Crumb>().crumbName
                == product.GameObject().GetComponent<Crumb>().crumbName
            )
            {
    ...
    public void SlideOrderTickets()
    {
        ...
        foreach (GameObject orderCard in currentOrders)
        {
            ...
            UnityEngine.Vector2 targetPosition =
                orderCardRectTransform.localPosition + rightwardMove;
            StartCoroutine(ActualSlide(targetPosition, orderSlideTime, orderCardRectTransform));
        }
    }

    IEnumerator ActualSlide(
        UnityEngine.Vector2 targetPosition,
        float duration,
        RectTransform orderCardRectTransform
    )
    {
        ...
        {
            orderCardRectTransform.localPosition = UnityEngine.Vector2.Lerp(
                startPosition,
                targetPosition,
